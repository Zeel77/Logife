...
const page = () => {
 ...
 const [currentStep, setCurrentStep] = useState('move')
  ...
  const [vesselDetail, setVesselDetail] = useState({})
    ...
    useEffect(() => {
      const getJobDetails = async ()=>{
        var {data} = await axios.get(`https://localhost:7127/api/jobdetails/id?jid=${jobId}`, {
            headers: {
        ...
        var {data} = await axios.get(`https://localhost:7127/api/vessel/name?name=${data.data.vesselName}`, {
            headers: {
                'Authorization': `Bearer ${sessionStorage.getItem('authtoken')}`
            }
        });
    ...
    const stepClasses = step => {
        return `flex flex-col gap-2 items-center justify-center text-${step === currentStep || stepsOrder.indexOf(step) < stepsOrder.indexOf(currentStep) ? 'green-600' : 'slate-400'}`;
    };
    const stepsOrder = ['assign', 'move', 'arrive', 'start'];
